...
CREATE TABLE Contact (
    id_contact NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    id_property NUMBER,
    date_contact DATE DEFAULT SYSDATE,
    name VARCHAR2(50),
    phone VARCHAR2(9),
    email VARCHAR2(50), 
...
-- Procedimientos de contact
CREATE OR REPLACE PROCEDURE Insert_Contact (
    p_id_property IN NUMBER,
    p_name IN VARCHAR2,
    p_phone IN VARCHAR2,
    p_email IN VARCHAR2,
...
IS
BEGIN
    INSERT INTO Contact (id_property, name, phone, email, message)
    VALUES (p_id_property, p_name, p_phone, p_email, p_message);
...
-- Procedimientos de favoriteProperty
CREATE OR REPLACE PROCEDURE Delete_Favorite_Property (
    p_id_favorite_property IN NUMBER
)
IS
BEGIN
    DELETE FROM Favorite_Property
    WHERE id_favorite_property = p_id_favorite_property;
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error al eliminar el elemento: ' || SQLERRM);
END;


CREATE OR REPLACE PROCEDURE Add_Favorite_Property (
    p_id_property IN NUMBER,
    p_id_user IN NUMBER
)
IS
BEGIN
    INSERT INTO FAVORITE_PROPERTY(id_property, id_user)
    VALUES (p_id_property, p_id_user);
    COMMIT;
...
DROP PROCEDURE ADD_FAVORITE_PROPERTY;
...
BEGIN
    INSERT INTO Agent (salary, experience, id_user, hire_date)
    VALUES (p_salary, p_experience, p_id_user, p_hire_date);
    
    COMMIT;
    
    DBMS_OUTPUT.PUT_LINE('Agente insertado correctamente.');
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error al insertar agente: ' || SQLERRM);
END;
...
-- Procedimiento agregar imagen propiedad
CREATE OR REPLACE PROCEDURE Add_Property_Image(
    p_id_property NUMBER,
    p_image VARCHAR
)
IS
BEGIN
    INSERT INTO Image_Property (
        id_property,
        image
    ) VALUES (
        p_id_property,
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Nueva imagen de propiedad agregada correctamente.');
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error al agregar la imagen de propiedad: ' || SQLERRM);
END Add_Property_Image;

CREATE OR REPLACE PROCEDURE Delete_Image_Property (
    p_id_image_property IN NUMBER
)
IS
BEGIN
    DELETE FROM Image_Property
    WHERE id_image_property = p_id_image_property;
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No se encontró ningún registro con el ID especificado.');
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error al eliminar el registro: ' || SQLERRM);
END;
...
-- Procedimiento agregar característica propiedad
CREATE OR REPLACE PROCEDURE Add_Property_Characteristics(
    p_number_rooms NUMBER,
    p_numbers_bathrooms NUMBER,
...
IS
BEGIN
    INSERT INTO Characteristics_Property (
        number_rooms,
        number_bathrooms,
        description,
        garage,
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Nuevas características de propiedad agregadas correctamente.');
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error al agregar las características de propiedad: ' || SQLERRM);
END Add_Property_Characteristics;
-- ***************************************************************************************
...
-- Procedimiento agregar direccion
CREATE OR REPLACE PROCEDURE Add_Address(
    p_street VARCHAR,
    p_province VARCHAR,
    p_canton VARCHAR,
    p_others_signs VARCHAR
)
IS
BEGIN
    INSERT INTO Address (
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error al agregar la dirección: ' || SQLERRM);
END Add_Address;




-- Procedimiento agregar propiedad
...
IS
    v_id_address NUMBER;
    v_id_characteristics NUMBER;
    v_id_property NUMBER;
BEGIN
    ...
    RETURNING id_characteristics_property INTO v_id_characteristics;
    ...
    RETURNING id_property INTO v_id_property;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Nueva propiedad agregada correctamente.');
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error al agregar la propiedad: ' || SQLERRM);
END Agregar_Propiedad;
-- ***************************************************************************************
...
CREATE OR REPLACE PROCEDURE Add_Rol (
    p_name VARCHAR2,
    p_id_user NUMBER
)
IS
